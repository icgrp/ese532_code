...
void compute_hash_flags(hls::stream<parallel_flags_t>& flag_stream,
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=t_size/pf
		...
		compute_hash_flags_loop: for (unsigned int j = 0; j < PARALLELISATION;
				j++) {
#pragma HLS UNROLL
			...
			bool inh1 = (!doc_end)
					&& (bloom_filter_local[j][hash1 >> 5]
							& (1 << (hash1 & 0x1f)));
			unsigned hash2 = (hash_pu + hash_lu) & hash_bloom;
			bool inh2 = (!doc_end)
					&& (bloom_filter_local[j][hash2 >> 5]
							& (1 << (hash2 & 0x1f)));
...
void compute_hash_flags_dataflow(ap_uint<512>* output_flags,
...
void runOnfpga(ap_uint<512>* output_flags, ap_uint<512>* input_words,
		unsigned int* bloom_filter, unsigned int total_size, bool load_filter) {
#pragma HLS INTERFACE ap_ctrl_chain port=return            bundle=control
#pragma HLS INTERFACE m_axi         port=output_flags      bundle=maxiport0   offset=slave
#pragma HLS INTERFACE m_axi         port=input_words       bundle=maxiport1   offset=slave
#pragma HLS INTERFACE m_axi         port=bloom_filter      bundle=maxiport0   offset=slave
...
#pragma HLS ARRAY_PARTITION variable=bloom_filter_local complete dim=1
	...
	if (load_filter == true) {
		read_bloom_filter: for (int index = 0; index < bloom_filter_size;
				index++) {
#pragma HLS PIPELINE II=1
	...
	compute_hash_flags_dataflow(output_flags, input_words, bloom_filter_local,
			total_size);
}
}
