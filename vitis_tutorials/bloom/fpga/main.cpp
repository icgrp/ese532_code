...
int main(int argc, char** argv)
{
    ...
    chrono::high_resolution_clock::time_point t1, t2, t3, t4;
    	...
    	// Create buffers
    	cl::Buffer buffer_bloom_filter(context,  CL_MEM_READ_ONLY, bloom_filter_size*sizeof(uint));
    	cl::Buffer buffer_input_doc_words(context, CL_MEM_READ_ONLY, total_size*sizeof(uint));
    	cl::Buffer buffer_output_inh_flags(context, CL_MEM_WRITE_ONLY, total_size*sizeof(char));

    	bloom_filter = (unsigned int *) q.enqueueMapBuffer (buffer_bloom_filter , CL_TRUE , CL_MAP_WRITE , 0, bloom_filter_size*sizeof(uint));
    	input_doc_words = (unsigned int *) q.enqueueMapBuffer (buffer_input_doc_words , CL_TRUE , CL_MAP_WRITE , 0, total_size*sizeof(uint));
    	unsigned char* output_inh_flags = (unsigned char *) q.enqueueMapBuffer (buffer_output_inh_flags , CL_TRUE , CL_MAP_READ , 0, total_size*sizeof(char));
    	...
    	for (unsigned i=0; i<size; i++) {
    	    ...
    	    for (unsigned i=0; i<(1L << bloom_size); i++) {
    	        ...
    	        t4 = chrono::high_resolution_clock::now();
    	            	chrono::duration<double> perf_all_sec_2  = chrono::duration_cast<duration<double>>(t4-t3);
    	            	printf(" Data setup time | %10.4f ms", 1000*perf_all_sec_2.count());
    	...
    	printf("\n");
        double mbytes_total  = (double)(size * sizeof(int)) / (double)(1000*1000);
